...
def load_database(database):
    ...
    try:
        ...
        c.execute(
            "CREATE TABLE results (id int, compid int, seasonid int, skierid int, competitionpart int,"
            " result int, distance text, teqnique text, time text, tourresult int, tour int)"
        )
        c.execute(
            "CREATE TABLE biathlon_results (id int, compid int, seasonid int, skierid int, competitionpart int,"
            " result int, distance text, teqnique text, time text, shooting text, tour int)"
        )
        c.execute(
            "CREATE UNIQUE INDEX result_index ON results(id, compid, seasonid, skierid, competitionpart)"
        )
        c.execute(
            "CREATE UNIQUE INDEX biathlon_result_index ON biathlon_results(id, compid, seasonid, skierid)"
        )
    ...
    for sqlstatement in json_data["results"]:
        if sqlstatement["teqnique"] == "Biathlon":
            c.execute(
                "insert into biathlon_results values ("
                + sqlstatement["id"]
                + ","
                + sqlstatement["compid"]
                + ", "
                + sqlstatement["seasonid"]
                + ", "
                + sqlstatement["skierid"]
                + ", "
                + sqlstatement["competitionpart"]
                + ", "
                + sqlstatement["result"]
                + ", '"
                + sqlstatement["distance"]
                + "', '"
                + sqlstatement["teqnique"]
                + "', '"
                + sqlstatement["time"]
                + "', '"
                + sqlstatement["shooting"]
                + "', '"
                + sqlstatement["tour"]
                + "')"
            )
...
def skidresults(skier, template, biathlon=False):
    ...
    for season in seasons:
        ...
        if biathlon:
            (
                ...
                stot,
                scountpallplatser,
            ) = get_biathlon_results(dbconn, skier, season)
        else:
            results, fvinster, fandra, ftredje, stot, scountpallplatser = get_results(
                dbconn, skier, season
            )
    ...
    return render_template(
        ...
        season_pallplatser=season_pallplatser,
        season_pallplatser_grouped=season_pallplatser_grouped,
        namn=skier,
    )
...
def index():
    ...
    fpallplatser, ftot, ftotcomp, fcountpallplatser = get_pallplatser_index(
        dbconn, "Frida Bergqvist"
    )
    ...
    (
        ...
        b_ftotcomp,
        b_fcountpallplatser,
    ) = get_pallplatser_biathlon_index(dbconn, "Frida Bergqvist")

    tpallplatser, ttot, ttotcomp, tcountpallplatser = get_pallplatser_index(
        dbconn, "Tilde Bergqvist"
    )

    return render_template(
        "index.html",
        fpallplatser=fpallplatser,
        ftot=ftot,
        fcountpallplatser=fcountpallplatser,
        tpallplatser=tpallplatser,
        ttot=ttot,
        tcountpallplatser=tcountpallplatser,
        b_fpallplatser=b_fpallplatser,
        b_ftot=b_ftot,
        b_fcountpallplatser=b_fcountpallplatser,
        ftotcomp=ftotcomp,
        ttotcomp=ttotcomp,
    )
